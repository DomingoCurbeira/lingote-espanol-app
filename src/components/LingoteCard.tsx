...
export const LingoteCard = ({ lingote, onSelect }: Props) => {
  ...
  <>
      <motion.div 
          ...
          className={`bg-white rounded-[3.5rem] overflow-hidden shadow-2xl border border-gray-100 flex flex-col group h-full transition-all relative ${!disponible ? 'pointer-events-none' : ''}`}
      >
      {/* Sello de Agotado (Capa superior, NO afectada por grayscale) */}
      {!disponible && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/20 backdrop-blur-[1px]">
              <div className="bg-red-600 text-white font-black text-sm px-6 py-2 rounded-full uppercase italic rotate-[-12deg] shadow-2xl border-2 border-white ring-4 ring-red-600/20 animate-in zoom-in duration-300">
                  ¡AGOTADO! ❌
              </div>
          </div>
      )}

      {/* Contenido del Producto (Afectado por grayscale si está agotado) */}
      <div className={`flex flex-col flex-grow transition-all duration-500 ${!disponible ? 'grayscale opacity-40 blur-[0.5px]' : ''}`}>
          <div className={`relative group bg-white rounded-t-[3.5rem] overflow-hidden shadow-xl transition-all ${
              disponible ? 'hover:shadow-2xl hover:-translate-y-1' : ''
          }`}>
